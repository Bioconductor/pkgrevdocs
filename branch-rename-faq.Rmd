...
Bioconductor developers will have to rename default branches on their __local__
repositories to 'devel'. Optionally (and highly encouraged), GitHub and other
local copies of affected repositories, including those from collaborators,
should have their branches renamed.
...
to simply work without specifying additional parameters when pushing to GitHub
and Bioconductor.
...
__Note__. Change 'master' to 'main' in the code above if the repository has
...
**NOTE**: The following sections are for eager developers who would like to
make changes to their local repositories _before_ the Bioconductor git branch
rename (now).

#### Sync repositories for early adopters
...
#### Rename devel branch on GitHub for early adopters
...
#### Pushing to Bioconductor for early adopters
