...
* Subscribe to the [bioc-devel](https://bioconductor.org/help/mailing-list/)
  mailing list
...
The [HubPub][] package is helpful for creating a template for a hub package. The
vignette [Create A Hub Package][] provides full details.
...
* The package will be submitted to the _Bioconductor_ build
  ...
  the package conforms to _Bioconductor_ [BiocCheck][] standards. The system
  will perform these steps using the ['devel'
  version](#use-devel) of _Bioconductor_, on three platforms
...
* Packages are then built by the _Bioconductor_ nightly build
  ...
  [build report] for how often the different package types are build. The
...
[HubHowTo]: https://bioconductor.org/packages/devel/bioc/vignettes/HubPub/inst/doc/CreateAHubPackage.html
