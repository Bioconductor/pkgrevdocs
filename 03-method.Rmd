...
The following section will review some important notes regarding fields of the `DESCRIPTION` file and associated files.
...
The license field should preferably refer to a standard license (see [wikipedia](http://en.wikipedia.org/wiki/Comparison_of_free_software_licences)) using one of R’s standard specifications.
...
If the installation process is non-trivial, a top-level `README` file should be included to document the process.
...
This field directs users to source code repositories, additional help resources, etc; details are provided in [Writing R Extensions](https://cran.r-project.org/doc/manuals/r-release/R-exts.html), `RShowDoc("R-exts")`.
